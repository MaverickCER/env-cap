...
`generate:env` (it only includes `src/env.ts`). They exist purely as a
...
for the migration path itself, and `examples/composable-boilerplates` /
`examples/paypal-addon` for working examples of the fully split model.
...
`npm start` runs `generate:env` (writing `src/generated/env.manifest.ts` and
`docs/ENVIRONMENT.md` -- neither is gitignored; both are committed on purpose
...
`examples/composable-boilerplates` and `examples/paypal-addon`), but that's an
adoption target, not a requirement. A single `createEnv()` call -- like the
