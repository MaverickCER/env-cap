...
//   processor and no validator -- see PERFORMANCE.md's "Runtime fixture
...
const ARCHETYPES = [
  {
    name: "LIST_OF_TAGS",
    schemaSource: `{ processor: processors.array(), validator: validators.unique() }`,
  ...
  },
  {
    ...
    schemaSource: `{ processor: processors.date(), validator: validators.future() }`,
  ...
  },
  {
    ...
    schemaSource: `{ processor: processors.string(), validator: validators.oneOf(["dev", "staging", "production"]) }`,
    description: (i) => `Deployment tier for entry #${i}.`,
  },
  {
    name: "MAX_CONNECTIONS",
    schemaSource: `{ processor: processors.integer(), validator: validators.all(validators.min(1), validators.max(1000)) }`,
    description: (i) => `Maximum connection pool size for entry #${i}.`,
  },
  {
    name: "FEATURE_DEBUG_MODE",
    schemaSource: `{ processor: processors.boolean() }`,
    description: (i) => `Debug mode flag for entry #${i}.`,
  },
  {
    name: "SERVICE_ENDPOINT_URL",
    schemaSource: `{ processor: processors.url() }`,
  ...
  },
  {
    ...
    schemaSource: `{ processor: processors.json(), validator: (value) => (value && typeof value === "object") || "Expected an object." }`,
