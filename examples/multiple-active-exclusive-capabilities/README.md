...
capabilities as [`examples/composable-boilerplates`](../composable-boilerplates/) --
...
and docs, exactly as `examples/composable-boilerplates` already demonstrates for
the postgres-active, mongodb-dormant default.

## Why this is its own example, not a variation on an existing one

`examples/composable-boilerplates`'s own README documents this exact failure as
...
test ([`test/examples/multiple-active-exclusive-capabilities.test.ts`](../../test/examples/multiple-active-exclusive-capabilities.test.ts))
