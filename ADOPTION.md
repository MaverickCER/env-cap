...
| Warn, not throw, by default                                 | Adopting incrementally in a large codebase doesn't require fixing every finding on day one; `onIncompatibility: "throw"`/`--strict` opts into a stricter CI gate for the one provable error category (manifest compatibility) once a team is ready. Documentation/ownership issues never throw at all — they're `Finding`s on the evidence artifact, for a team to gate on itself | [0005](specs/decisions/0005-warn-not-throw-by-default.md), [0038](specs/decisions/0038-always-computed-evidence-model-backs-generate-check-docs.md) |
