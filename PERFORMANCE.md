...
Canonical documentation for [`performance-runtime/`](examples/performance-runtime/) and
[`performance-buildtime/`](examples/performance-buildtime/). Each example's own README covers what's
specific to it; this file covers everything shared.
...
[`basic-node/src/env.ts`](examples/basic-node/src/env.ts)) and then runs one `validateEnv()` call. Each
sample is a brand-new process — zero warmup by construction, since a cold process can't be
"warmed." The parent measures total wall-clock time per spawn; the child self-reports how much of
that was its own `createEnv` phase vs. its own `validateEnv` phase. `totalMs` minus
...
See [`performance-buildtime/README.md`](examples/performance-buildtime/README.md) for what each measures.
...
From the repo root, this drives both examples via `scripts/run-benchmarks.mjs`. Each example can
