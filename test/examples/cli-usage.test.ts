...
import {
  compareGoldenArtifacts, examplesRoot, isInstalled, normalizeExampleCliOutput, normalizeExampleJsonOutput, runScript,
} from "./support.js";
...
async function readGolden(relativePath: string): Promise<string> {
  return fs.readFile(path.join(examplesRoot, EXAMPLE, "expected", relativePath), "utf8");
}
...
describe(EXAMPLE, () => {
  it.skipIf(!installed)(
    ...
    async () => {
      const output = normalizeExampleCliOutput(runScript(EXAMPLE, "generate:env"), EXAMPLE);
  ...
  it.skipIf(!installed)(
    ...
    async () => {
      const output = normalizeExampleCliOutput(runScript(EXAMPLE, "verify:env"), EXAMPLE);
  ...
  it.skipIf(!installed)("generate:env:json emits a --json envelope matching its golden report.json, compared as parsed JSON so key ordering can't cause spurious failures", async () => {
    ...
    const actual = normalizeExampleJsonOutput(runScript(EXAMPLE, "generate:env:json"), EXAMPLE);
    const expected: unknown = JSON.parse(await readGolden("cli/report.json"));
