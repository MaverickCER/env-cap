...
describe(EXAMPLE, () => {
  it.skipIf(!installed)(
    ...
    () => {
      ...
      expect(stdout).toContain("Stripe key (from @examples/tsconfig-aliases): sk_test_fake_1234567890");
    },
  );
  ...
  it.skipIf(!installed)(
    ...
    async () => {
      ...
      expect(report).toContain("| billing (`src/features/billing/env.schema.ts`) | billing-team | 1 | src/app.ts | 1 |");
