...
describe(EXAMPLE, () => {
  it.skipIf(!installed)(
    'example:server validates with activeContexts: ["server"] and refuses the client-context variable',
    () => {
      const stdout = runScript(EXAMPLE_DIR, "example:server")
      ...
      expect(stdout).toContain(
        'PUBLIC_API_URL (context: "client", did not match) -- reading it throws:',
      )
  ...
  it.skipIf(!installed)(
    'example:client validates with activeContexts: ["client"] and refuses the server-context variable',
    () => {
      const stdout = runScript(EXAMPLE_DIR, "example:client")
      ...
      expect(stdout).toContain(
        'DATABASE_URL (context: "server", did not match) -- reading it throws:',
      )
      expect(stdout).toContain("has not been validated yet")
    },
  )
